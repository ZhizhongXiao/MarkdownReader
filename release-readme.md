...
1. 下载 `MarkdownReader.exe`，放入可写目录后双击运行。
2. 需要 Windows 10/11（64 位）及 Microsoft Edge WebView2 Runtime。
3. 已内置 Python、Node.js 和渲染依赖，无需另行安装开发环境。
...
- 同名输出 HTML 会被覆盖。
- 阅读器样式、脚本和公式资源内置于 HTML；文档引用的图片等外部资源不会自动复制或嵌入，需保证输出位置仍能访问。
...
本次发布统一项目名称、更新示例文档和明暗模式截图。可使用随附的 `SHA256SUMS.txt` 核验 EXE。
