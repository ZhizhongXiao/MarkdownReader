...
- 2026-09-26（Phase 8 audit follow-up #2：registry 只认 JSON object，`missing` / `invalid` 边界写进契约）：上一轮收口后又发现两处边角输入上的口子，正常主题链路与产物均不受影响。
  * **非 object 的 `metadata.json` 不再击穿 registry（F1，阻断）**：`viewer_assets._metadata_at()` 声明返回 `dict`，但 `json.load()` 对 `"broken"` / `[1]` 这类**合法** JSON 返回的是 `str` / `list`，`_scan_theme_ids()` 随即 `.get()` → `AttributeError`。后果是三条入口一起崩：GUI 启动读主题状态（`BridgeApi.get_theme_state()`）、转换解析主题选择（`resolve_theme_selection()`）与 loader 侧的 `viewer_assets.validate_theme()` / `resolve_theme_chain()` 都会直接抛 `AttributeError`（前者本应给出 `ValueError`），而不是把这份 metadata 当作不可用。现在只有**非空** JSON object 才算 metadata：非 object 与「解析失败」同类 —— 返回 `{}` 并记一条含实际类型名的 warning；空的 object 与「没有 metadata.json」同类 —— 安静返回 `{}`、不新增 warning（缺失 metadata 从不 warning）。两条路径的落点一致：目录因此不算主题，`configured` 里的 id 归 `missing`。它落实的仍是 Phase 7 既有契约（未安装 / 损坏的 metadata 是忽略 + warning，不是崩溃）。
