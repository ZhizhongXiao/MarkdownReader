...
| K26 | **renderer 选择桥（Cutover C2）**：`render_markdown_node(..., renderer_version="v1" \| options=None)` 是**显式**选择，默认 `v1`（production 默认未切换）。选 `v2` 时：① `protocol_version` 必须等于 2，**不做协议探测**（拿到别的形状直接失败，不"看形状猜"）；② **不静默回退 v1**（失败就失败）；③ 返回**完整 envelope**（`html` / `headings` / `features` / `warnings` / `resources`），不压缩成 v1 的 `{html, headings, assets, warnings}`；④ Node 下限 **major >= 18**，常量 `MINIMUM_NODE_MAJOR` 只有一处来源（`core/renderer_v2.py`），`packaging/MarkdownReader.spec` 在既有 exact-version + SHA-256 之后对已记录的 bundled Node 断言同一常量，**v1 行为不变**；⑤ `renderer/dist/renderer.cjs` 缺失 → actionable failure（提示 `cd renderer; npm ci; npm run build`）。`options` 只对 v2 生效，v1 收到非空 options 报 `ValueError`。Node 版本全项目只读一次（`probe_node_version`，v1 校验与 v2 下限共用），v2 桥不自行读版本；v2 冒烟离线且要求 `dist/katex` 真的参与渲染 | `tests/test_node_runtime.py`（11 项：探针共用一次 / 17 拒 / 18 与 24 收 / 版本不可解析拒 / 默认与显式 v1 从不触碰 v2 / v1 拒绝 options 与未知版本）+ `tests/test_renderer_v2_bridge.py`（13 项：真实调用 + 各通道原样透传 + 协议与形状拒绝 + error envelope 传播 + 不回退 v1 的反证 + 每进程一次冒烟） |
