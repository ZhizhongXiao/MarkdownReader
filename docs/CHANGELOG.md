...
- 面向人的提示不再显示 URL 编码：未加入转换清单的链接与无法内嵌的图片，提示里此前是 `%E7%94%B2%E7%BB%84/%E4%B8%80%E5%8F%B7.md` 这类机器形式，现在显示作者写的 `甲组/一号.md`，query 与 fragment 一并还原，文件名中字面的 `%`（`100%25.md`）也还原为 `100%.md`；无法解码的畸形序列原样保留且不影响渲染。HTML 中的 `href`/`src` 保持规范的百分号编码不变。
