...
- 正文改为左对齐。两端对齐在中文里没有词间空格可拉伸，只能拉开字与字之间的距离：一段混排中文、文件名和网址的文字因此出现明显空隙。
- 超长链接与裸文件名允许在容器内折行。内容区本身会裁掉横向溢出，此前它们会被截断而不是换到下一行。
- 打印改为「白纸 + 跟随正文的主题块」：纸面统一为白，主题的浅蓝底与左右两条框线移到正文列上，长度随正文从第一行到最后一行；关闭「背景图形」时只丢蓝底，框线仍在，因为边框不受该开关影响。
- 图片灯箱支持滚轮缩放，范围为适应屏幕到放大 6 倍；遮罩不移动，任意缩放级别下点击仍可关闭。
- 同一时刻只允许一个原生对话框：对话框打开期间其余对话框控件被禁用，桥接层再用非阻塞锁拒绝重叠请求，消除两次对话框同时存在引起的偶发失败。
- 转换完成后以 `file://` URI 打开结果文件。此前传入裸 Windows 路径，由系统 `.html` 关联决定打开方式，关联异常时可能不打开或打开别的东西。
