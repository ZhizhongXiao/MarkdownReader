...
MDViewer 保持“下载一个 EXE 即可运行”的无安装器便携形态。打包升级只改善依赖锁定、构建隔离、WebView2 失败提示和发布可追溯性，不改为安装器或正式 `onedir` 产品。

项目进入以修复和微调为主的维护期：预计仍会修改 CSS、模板、兼容性处理和少量内部逻辑，但不再以增加大型功能为目标。因此应保留稳定、可复现的完整重打包流程；不增加安装器、后台更新服务、自更新器或二进制增量补丁。每次维护发布都生成新的完整 EXE，使用递增版本号和 SHA-256 标识，旧版本至少保留一个发布周期，以便出现回归时直接替换回退。

推荐目标：

- 使用 `uv` 管理项目专用 `.venv` 和 `uv.lock`，不再依赖全局 Python 包集合；
- 在 `pyproject.toml` 声明运行依赖，在 `build` 依赖组锁定 PyInstaller；
- 保留 PyInstaller `onefile`、`windowed`、启动图、图标和内置 Node.js；
- 将 `MDViewer.spec` 的 `upx` 改为 `False`，不使用 UPX；
...
该方案尚未写入依赖文件和 spec，以下仍是当前可用的既有构建命令。升级实施后，推荐构建入口调整为：

```powershell
uv lock --check
uv sync --locked --group build
uv run --locked --group build python -m PyInstaller `
  --clean --noconfirm `
...
若内置 Node 导致单文件启动释放时间、Defender 扫描或临时目录占用不可接受，可另行评估无安装器的 `MDViewer-portable.zip`/`onedir` 形态；在出现实测问题前不同时维护两套正式发布物。
...
`packaging/MDViewer.spec` 会在 `packaging/node/` 存在时自动把该目录打入 EXE。
运行时，程序会优先调用内置的 `node/node.exe`；如果没有内置 Node，则回退到系统
`PATH` 中的 `node`。
...
- `config.json` 在 `MDViewer.exe` 同级目录读取和写入
- 打包的只读资源从 PyInstaller 临时目录加载
- WebView2 数据保存在 `%LOCALAPPDATA%\MDViewer\WebView2`
...
- 构建使用与 `uv.lock` 一致的依赖，构建环境没有未声明包。
- `dist/MDViewer.exe` 可在干净目录启动。
- 目标目录和目标机器不需要 Python、uv 或虚拟环境。
- 没有系统 Node.js 时仍优先使用内置 `node/node.exe`。
- WebView2 Runtime 缺失时给出明确提示，不静默回退到旧渲染器。
- 首次转换后会在 EXE 同级目录写入 `config.json`。
- 单文件转换、文件夹批量转换、索引页和三套模板均可用。
- 转换过程中不会弹出额外命令行窗口。
- 发布记录包含 Python、uv、PyInstaller、pywebview、Node.js 版本，`uv.lock` 哈希和 EXE SHA-256。
- 新版与上一个发布版使用不同文件名或版本目录，旧版可直接恢复；不覆盖用户已有的 `config.json`。
